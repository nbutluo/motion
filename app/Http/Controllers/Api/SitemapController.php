    ...
    public function createSiteMap()
    {
        ...
        try {
            ...
            foreach ($newsCategory as $newsCate) {
                ...
                Sitemap::firstOrCreate(['url' => '/blogs/' . $newsCate->title], ['name' => 'blog分类', 'method' => 1, 'origin' => '/loctek/category/blog-list', 'type' => 9]);
            }
            //生成新闻链接
            $news = Blog::select(['post_id', 'title', 'category_id'])->get();
            foreach ($news as $new) {
                $news_category[$new->category_id] = str_replace(' ', '-', $news_category[$new->category_id]);
                Sitemap::firstOrCreate(['url' => '/blogs/' . $news_category[$new->category_id] . '/' . str_replace(' ', '-', $new->title)], ['name' => '文章详情', 'method' => 1, 'origin' => '/loctek/blogDetail/' . $new->title, 'type' => 10]);
            }
